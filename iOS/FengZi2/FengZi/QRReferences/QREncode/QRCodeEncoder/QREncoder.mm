...
+ (void)drawDotAt:(int)x 
                y:(int)y 
           offset:(int)offset 
         colorRed:(unsigned char)red
       colorGreen:(unsigned char)green
        colorBlue:(unsigned char)blue
      pixelPerDot:(int)pixelPerDot 
   imageDimension:(int)imageDimension 
    ...
    for (int my = startY; my < endY; my++) {
        for (int mx = startX; mx < endX; mx+=bytesPerPixel) {
            rawData[bytesPerLine * my + mx    ] = red;    //red
            rawData[bytesPerLine * my + mx + 1] = green;    //green
            rawData[bytesPerLine * my + mx + 2] = blue;    //blue
...
+ (void)drawDotAt:(int)x y:(int)y offset:(int)offset intensity:(unsigned char)intensity pixelPerDot:(int)pixelPerDot imageDimension:(int)imageDimension rawData:(unsigned char*)rawData {
    ...
    [QREncoder drawDotAt:x 
                       y:y 
                  offset:offset 
               colorRed:intensity 
              colorGreen:intensity 
               colorBlue:intensity 
             pixelPerDot:pixelPerDot 
