...
+ (MediaInfo *)uploadImage:(NSData *)buffer{
    ...
    //NSString *action = [NSString stringWithFormat:@"%@/%@?userid=%d", API_URL_RICHMEDIA, path, [Api userId]];
      ...
      NSString *action = [NSString stringWithFormat:@"%@%@?userid=%d", API_FILE_SERVER, API_FILE_UPLOAD, [Api userId]];
                      ...
                      uuid:(NSString *)uuid
{
    ...
    NSString *serv =API_RICHMEDIA_SERVER;
    ...
    if (data.count > 0) {
        ...
        if ([Api kma]) {
            path = API_RICHMEDIA_SERVER API_MAKE_CODE; 
...
+ (MediaContent *)getContent:(NSString *)uuid{
    ...
    NSString *action = [NSString stringWithFormat:@"%@?id=%@", API_RICHMEDIA_SERVER API_GET_CODE, uuid];  
...
+ (ApiResult *)kmaUpload:(NSString *)pid
    ...
    NSString *action = [NSString stringWithFormat:@"%@", API_RICHMEDIA_SERVER API_MAKE_CODE];
