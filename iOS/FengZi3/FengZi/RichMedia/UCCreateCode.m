...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)xInfo {
    ...
    if ([mediaType isEqualToString:@"public.image"]){
        ...
        if (fileSize >= IOSAPI_MB(4)) {
            [iOSApi Alert:@"提示" message:@"上传影音文件不能超过4MB。"];
...
- (void)startUploadImage{
    ...
    HttpClient *hc = [[HttpClient alloc] initWithURL:API_FILE_SERVER API_FILE_UPLOAD timeout:10];
    ...
    if (tmpData == nil) {
        ...
        if (data.count > 0) {
            ...
            NSString *value = [data objectForKey:@"filetype"];
            if (value != nil) {
                info.key = value;
            }
            info.tinyName = [data objectForKey:@"fileurl"];
...
- (void)generateCode{
    ...
    ModelInfo *xRet= [[Api uploadModel:title content:desc url:nameTiny type:nType  uuid:xCode] retain];
    [iOSApi closeAlert];
    if (xRet.status != 0) {
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    label.text= [Api kma] ? @"制作专属码" : @"个性化内容生码";
