...
- (void)viewWillAppear:(BOOL)animated {
  ...
  //设置导航图片    
    ...
    if ([_items count] == 0) {
        ...
        NSMutableArray *result = [[DataBaseOperate shareData] loadMemberList:_page sid:param.sid];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
   ...
   // NSLog(@"加载");
    ...
    UIImage *image = [[UIImage imageNamed:@"cellbg.png"] toSize: CGSizeMake(284, 44)];
    ...
    EFileMemberList *obj = [_items objectAtIndex:pos];
    ...
    cell.imageView.image =[[UIImage imageWithContentsOfFile:path]toSize:CGSizeMake(50, 30)];
    /*
    if([Api fileIsExists:path])
    {
       cell.imageView.image =[UIImage imageWithContentsOfFile:path];  
    }
    else
    {
        if([Api fileIsExists:path2])
        {
           cell.imageView.image =[UIImage imageWithContentsOfFile:path2]; 
        }
        else
        {
        cell.imageView.image = [[UIImage imageNamed:@"unknown.png"] toSize:CGSizeMake(40, 40)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
   ...
   //跳转页面
    ...
    //跳转页面
        ...
        EFileMemberList *obj = [_items objectAtIndex:pos];
        EFileHYKInfo *nextView = [[EFileHYKInfo alloc] init];
        nextView.param = obj;
